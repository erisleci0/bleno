...
class BlenoBindings extends EventEmitter {
  ...
  init() {
    ...
    this._hci.on('encryptChangeComplete', this.onEncryptChangeComplete.bind(this));
    this._hci.on('leLtkRequest', this.onLeLtkRequest.bind(this));
    this._hci.on('leLtkRequestError', this.onLeLtkRequestError.bind(this));
    this._hci.on('leLtkReplyComplete', this.onLeLtkReplyComplete.bind(this));
    this._hci.on('leLtkNegReplyComplete', this.onLeLtkNegReplyComplete.bind(this));
  ...
  onEncryptChangeComplete(status, handle, encrypt) {
    if (this._handle === handle && this._aclStream) {
      this._aclStream.pushEncrypt(status === 0 && !!encrypt);
    }
  }

  onLeLtkRequest(handle, random, ediv) {
    if (this._handle !== handle || !this._aclStream ||
        !this._aclStream.hasPendingScEncryptionMaterial()) {
      return;
    }

    const validRandom = Buffer.isBuffer(random) &&
      random.length === 8 &&
      random.every((value) => {
        return value === 0;
      });
    const validEdiv = Number.isInteger(ediv) && ediv === 0;

    if (!validRandom || !validEdiv) {
      this.rejectScLtkRequest(handle);
      return;
    }
    ...
    try {
      ltk = this._aclStream.takePendingScLtk();
    } catch (error) {
      this.rejectScLtkRequest(handle);
      return;
    }
    ...
    try {
      this._hci.leLtkReply(handle, ltk);
    } catch (error) {
      this._aclStream.pushEncrypt(false);
    } finally {
      ltk.fill(0);
    }
  }

  onLeLtkRequestError(handle) {
    if (this._handle === handle && this._aclStream &&
        this._aclStream.hasPendingScEncryptionMaterial()) {
      this.rejectScLtkRequest(handle);
    }
  ...
  rejectScLtkRequest(handle) {
    try {
      this._hci.leLtkNegReply(handle);
    } catch (error) {
      // Pending SC state is still cleared below.
    } finally {
      this._aclStream.pushEncrypt(false);
    }
  }

  onLeLtkReplyComplete(status, handle) {
    if (this._handle === handle && this._aclStream && status !== 0) {
      this._aclStream.pushEncrypt(false);
    }
  }

  onLeLtkNegReplyComplete(status, handle) {
    if (this._handle === handle && this._aclStream) {
