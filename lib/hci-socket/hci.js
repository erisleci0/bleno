...
const LE_LTK_REPLY_CMD = OCF_LE_LTK_REPLY | OGF_LE_CTL << 10;
const LE_LTK_NEG_REPLY_CMD = OCF_LE_LTK_NEG_REPLY | OGF_LE_CTL << 10;

const HCI_OE_USER_ENDED_CONNECTION = 0x13;
const HCI_MAX_CONNECTION_HANDLE = 0x0eff;

const STATUS_MAPPER = require('./hci-status');

function assertConnectionHandle(handle) {
  if (!Number.isInteger(handle) ||
      handle < 0 || handle > HCI_MAX_CONNECTION_HANDLE) {
    throw new RangeError(
      'handle must be an integer from 0x0000 to 0x0eff'
    );
  }
}
...
class Hci extends EventEmitter {
  ...
  resetBuffers() {
    ...
    this._handleBuffers = {};
    this._aclOutQueue = [];
    this._pendingLeLtkReplyHandle = null;
    this._pendingLeLtkNegReplyHandle = null;
  }
  ...
  leLtkReply(handle, ltk) {
    ...
    if (this._pendingLeLtkReplyHandle !== null) {
      throw new Error('LE LTK Reply command is already pending');
    }

    const cmd = Buffer.alloc(22);
    // AclStream stores the LTK MSO-first; HCI carries it LSO-first.
    const hciLtk = Buffer.from(ltk).reverse();

    try {
      cmd.writeUInt8(HCI_COMMAND_PKT, 0);
      cmd.writeUInt16LE(LE_LTK_REPLY_CMD, 1);
      cmd.writeUInt8(0x12, 3);
      cmd.writeUInt16LE(handle, 4);
      hciLtk.copy(cmd, 6);

      this._pendingLeLtkReplyHandle = handle;
      debug('LE LTK Request Reply - writing');
      this._socket.write(cmd);
    } catch (error) {
      this._pendingLeLtkReplyHandle = null;
      throw error;
  ...
  leLtkNegReply(handle) {
    ...
    if (this._pendingLeLtkNegReplyHandle !== null) {
      throw new Error(
        'LE LTK Request Negative Reply command is already pending'
      );
    }
    ...
    try {
      cmd.writeUInt8(HCI_COMMAND_PKT, 0);
      cmd.writeUInt16LE(LE_LTK_NEG_REPLY_CMD, 1);
      cmd.writeUInt8(0x02, 3);
      cmd.writeUInt16LE(handle, 4);

      this._pendingLeLtkNegReplyHandle = handle;
      debug('LE LTK Request Negative Reply - writing');
      this._socket.write(cmd);
    } catch (error) {
      this._pendingLeLtkNegReplyHandle = null;
      throw error;
  ...
  onSocketData(data) {
    ...
    const isLeLtkRequest = data.length >= 4 &&
      data[0] === HCI_EVENT_PKT &&
      data[1] === EVT_LE_META_EVENT &&
      data[3] === EVT_LE_LTK_REQUEST;
    const completedCommand = data.length >= 6 &&
      data[0] === HCI_EVENT_PKT &&
      data[1] === EVT_CMD_COMPLETE ?
      data.readUInt16LE(4) : null;
    const isLeLtkCommandComplete =
      completedCommand === LE_LTK_REPLY_CMD ||
      completedCommand === LE_LTK_NEG_REPLY_CMD;

    if (isLeLtkRequest) {
      debug('onSocketData: LE LTK Request');
    } else if (isLeLtkCommandComplete) {
      debug('onSocketData: LE LTK command complete');
    } else {
    ...
    if (HCI_EVENT_PKT === eventType) {
      ...
      if (subEventType === EVT_DISCONN_COMPLETE) {
        ...
        if (handle > HCI_MAX_CONNECTION_HANDLE) {
          return;
        }
        ...
        this.emit('encryptChangeComplete', status, handle, encrypt);
      } else if (subEventType === EVT_CMD_COMPLETE) {
        if (data.length < 7) {
          if (isLeLtkCommandComplete) {
            this.processCmdCompleteEvent(
              completedCommand,
              0xff,
              Buffer.alloc(0)
            );
        ...
        debug('\t\tcmd = ' + cmd);
        debug('\t\tstatus = ' + status);
        if (cmd !== LE_LTK_REPLY_CMD &&
            cmd !== LE_LTK_NEG_REPLY_CMD) {
          debug('\t\tresult = ' + result.toString('hex'));
        }

        if (isLeLtkCommandComplete &&
            (data.length !== 9 || data[2] !== 6)) {
          this.processCmdCompleteEvent(cmd, 0xff, Buffer.alloc(0));
        } else {
        ...
        if (leMetaEventType === EVT_LE_LTK_REQUEST) {
          this.processLeLtkRequestEvent(data);
          return;
        }
  ...
  processCmdCompleteEvent(cmd, status, result) {
    if (cmd === RESET_CMD) {
    ...
    } else if (cmd === LE_LTK_REPLY_CMD) {
      const handle = this._pendingLeLtkReplyHandle;
      const validResult = handle !== null &&
        result.length === 2 &&
        result.readUInt16LE(0) === handle;
      const completionStatus = validResult ? status : 0xff;

      this._pendingLeLtkReplyHandle = null;

      if (handle !== null) {
        this.emit('leLtkReplyComplete', completionStatus, handle);
      }
    } else if (cmd === LE_LTK_NEG_REPLY_CMD) {
      const handle = this._pendingLeLtkNegReplyHandle;
      const validResult = handle !== null &&
        result.length === 2 &&
        result.readUInt16LE(0) === handle;
      const completionStatus = validResult ? status : 0xff;
      ...
      if (handle !== null) {
        this.emit('leLtkNegReplyComplete', completionStatus, handle);
  ...
  processLeLtkRequestEvent(packet) {
    let handle = null;

    if (packet.length >= 6) {
      handle = packet.readUInt16LE(4);
    }
    ...
    if (packet.length !== 16 ||
        ...
        packet[2] !== 13 ||
        packet[3] !== EVT_LE_LTK_REQUEST ||
        handle === null ||
        handle > HCI_MAX_CONNECTION_HANDLE) {
      this.emit('leLtkRequestError', handle);
      return;
    }

    // No Status field: handle starts immediately after the subevent code.
    // Random stays in HCI LSO-first order; SC only accepts the all-zero value.
    const random = Buffer.from(packet.slice(6, 14));
    const ediv = packet.readUInt16LE(14);

    this.emit('leLtkRequest', handle, random, ediv);
  }
